<section class="container-fluid">
    <img width="100%" src="{{asset('images/1571211179VbwryC6BF6G3d1l.jpg')}}">
    <div class="d-flex filters filter-button-group w-100 justify-content-md-around pb-1">
        <div  class="active" data-filter="*">Tất cả</div>
        <div data-filter=".iso-1">Thiết kế Logo</div>
        <div data-filter=".iso-2">Nhận diện thương hiệu</div>
        <div data-filter=".iso-3">Ấn phẩm Marketing</div>
        <div data-filter=".iso-4">Bao bì sản phẩm</div>
        <div  data-filter=".iso-5">Thiết kế & Lập trình Website</div>
    </div>
    <div class="grid isotope-grid">
        <div class="grid-sizer"></div>
        <div class=" iso-1 iso-2 iso-3 grid-item">
            <img class="p2" src="{{asset('images/isotope@2x.jpg')}}">
        </div>
        <div class="iso-2 iso-3 iso-4 grid-item">
            <img class="p2" src="{{asset('images/isotope@2x.jpg')}}">
        </div>
        <div class="iso-3 iso-4 iso-5 grid-item">
            <img class="p2" src="{{asset('images/isotope@2x.jpg')}}">
        </div>
        <div class="grid-item--width2 iso-4 iso-5 iso-1 grid-item">
            <img class="p2" src="{{asset('images/isotope@2x.jpg')}}">
        </div>
        <div class="grid-item--width2 iso-1 iso-2 iso-3 grid-item">
            <img class="p2" src="{{asset('images/isotope@2x.jpg')}}">
        </div>
        <div class="iso-2 iso-3 iso-4 grid-item">
            <img class="p2" src="{{asset('images/isotope@2x.jpg')}}">
        </div>
        <div class="iso-3 iso-4 iso-5 grid-item">
            <img class="p2" src="{{asset('images/isotope@2x.jpg')}}">
        </div>
        <div class="iso-4 iso-5 iso-1 grid-item">
            <img class="p2" src="{{asset('images/isotope@2x.jpg')}}">
        </div>
        <div class="iso-1 iso-2 iso-3 grid-item">
            <img class="p2" src="{{asset('images/isotope@2x.jpg')}}">
        </div>
    </div>
    <div class="container text-right">
        <a href="#!" class="underline see-more">See more</a>
    </div>
</section>